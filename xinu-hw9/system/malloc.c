...
void *malloc(ulong size)
{
    ...
    //pmem->next = NULL;
	pmem->next = (void *)&(pmem->next);	
	
    pmem->length = size;

	//Garbage collector
	localPcb = &proctab[currpid[getcpuid()]];
	memMark = getmem(sizeof(struct memblock));
	if((void *)SYSERR == memMark){
		freemem(pmem, size);
        return NULL;
    }
	memMark->next = localPcb->myblocks;
	memMark->length = (ulong)pmem;
	localPcb->myblocks = memMark;
