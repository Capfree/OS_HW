...
syscall kgetc(void)
{
    ...
    if(bufSize > 0){
        //c = ungetArray[bufSize - 1];
        //ungetArray[--bufSize] = NULL;
        
        return ungetArray[--bufSize];
    }
...
syscall kcheckc(void)
{
    ...
    //if(ungetArray[0] != NULL)return TRUE;
 ...
 * @return c on success, false on failure.
 */
syscall kungetc(unsigned char c)
{
    ...
    if(bufSize >= UNGETMAX){
        kprintf("WARNING: unget buffer received more than 10 characters!\r\n");
        return FALSE;
    }else{
       /* int i;
        for(i = bufSize; i > 0; --i){
...
syscall kputc(uchar c)
{
    ...
    if(PL011_FR_TXFE & regptr->fr){
        regptr->dr = c;
        return c;
    }else{
        while(PL011_FR_TXFF & regptr->fr){
